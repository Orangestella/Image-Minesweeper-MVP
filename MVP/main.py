...
class MinesweeperGame:
    ...
    def build_buttons(self):
        ...
        self.settings_buttons = [
            ...
            Button((380, 40, 120, 38), "Clear cache", self.clear_cache, "danger"),
    ...
    def clear_cache(self):
        ...
        for attr, label in RESOURCE_FIELDS:
            path = getattr(self, attr)
            if path and self.path_is_inside(path, CACHE_DIR):
                setattr(self, attr, None)
                cleared.append(label)
        ...
        self.ensure_storage()
        self.reload_resources_from_paths()
        self.save_settings()
        if cleared:
            self.message = f"Cache cleared; cached settings reset: {', '.join(cleared)}"
        else:
            self.message = "Cache cleared"

    def path_is_inside(self, path, root):
        try:
    ...
    def reload_resources_from_paths(self):
        self.cover_image_source = self.load_image_source(self.cover_image_path)
        self.reveal_image_source = self.load_image_source(self.reveal_image_path)
        self.update_board_layout()
        self.win_sound = self.load_sound(self.win_audio_path)
    ...
    def draw_full_reveal_board(self):
        pygame.draw.rect(self.screen, (55, 61, 72), self.board_rect)
        if self.reveal_image:
            self.screen.blit(self.reveal_image, self.board_rect)
        pygame.draw.rect(self.screen, (10, 13, 18), self.board_rect, 1)
